...
<%--                </li>--%>
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="scenicDropdown">
                        ...
                        <li><a class="dropdown-item" href="scenic-list-recommend.jsp">景点推荐</a></li>
...
<div class="container mt-4">
    <h3 class="section-title">所有路线 </h3>
    ...
    <div class="row" id="routeListContainer">
        <c:forEach var="route" items="${routeList}" varStatus="status">
            <div class="col-md-3 mb-4 route-card"
...
<%--                        <p class="card-text text-muted">${route.description}</p>--%>
                        <div class="d-flex justify-content-between">
                            <button class="btn btn-sm btn-favorite btn-action"data-route-id="${route.id}" onclick="toggleFavorite(this)">
                                <i class="bi bi-heart"></i> 收藏
                            </button>
                            <button class="btn btn-sm btn-like btn-action"data-route-id="${route.id}" onclick="toggleLike(this)">
                                <i class="bi bi-hand-thumbs-up"></i>点赞
                                <span class="like-count">${route.likeCount}</span>
                            </button>
                            <button class="btn btn-sm btn-primary btn-action" data-id="${route.id}" onclick="showCommentBox(this)">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.btn-favorite').each(function() {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/route/status/' + routeId, // 假设后端接口返回收藏状态
                type: 'GET',
                success: function(response) {
                    if (response.code === 200) {
    ...
    function toggleLike(button) {
        ...
        const routeId = likeButton.data('route-id');
        ...
        $.ajax({
            url: isLiked
                ? '${pageContext.request.contextPath}/route/unlike' // 取消点赞
                : '${pageContext.request.contextPath}/route/like', // 点赞
            type: 'POST',
    ...
    function toggleFavorite(button) {
        ...
        const routeId = favoriteButton.data('route-id'); // 获取景点ID
        const isFavorited = favoriteButton.hasClass('active'); // 当前是否已收藏

        // 检查 routeId 是否为 undefined
        if (routeId === undefined) {
            console.error('routeId is undefined for button:', button);
            alert('无法获取路线ID，请重试');
            return;
        }
        ...
        $.ajax({
            url: url,
            type: 'POST',
            contentType: isFavorited ? 'application/json' : undefined,
            data: data,
            success: function(response) {
                if (response.code !== 200) {
                    // 操作失败，回滚UI状态
