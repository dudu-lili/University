...
<div class="container mt-4">
    ...
    <div class="row" id="scenicListContainer">
        <c:forEach var="scenic" items="${recommendedScenics}" varStatus="status">
            <div class="col-md-3 mb-4 scenic-card"
                 data-index="${status.index}"
                 data-province="${scenic.province}"
                 data-city="${scenic.city}"
                 data-name="${scenic.name}"
                 data-scenic-id="${scenic.id}"><!-- 添加 data-scenic-id 属性4.25 -->
                <div class="card h-100">
<%--                    <div>--%>
<%--                        <p>ID: ${scenic.id}, Name: ${scenic.name}, Cover Image: ${scenic.coverImage}</p>--%>
<%--                    </div>--%>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between">
                            <button class="btn btn-sm btn-favorite btn-action" data-scenic-id="${scenic.id}" onclick="toggleFavorite(this)">
                                <i class="bi bi-heart"></i> 收藏
                            </button>
                            <button class="btn btn-sm btn-like btn-action" data-scenic-id="${scenic.id}" onclick="toggleLike(this)">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.btn-favorite').each(function() {
            ...
            $.ajax({
                url: '${pageContext.request.contextPath}/scenic/status/' + scenicId, // 假设后端接口返回收藏状态
                type: 'GET',
                success: function(response) {
                    if (response.code === 200) {
                        const isFavorited = response.data.favorited;
                        favoriteButton.toggleClass('active', isFavorited); // 已收藏则添加active类
                    }
                }
            });
        });
    ...
    function toggleLike(button) {
        ...
        const scenicId = likeButton.data('scenic-id');
        const likeCountElement = likeButton.find('.like-count');
        const currentCount = parseInt(likeCountElement.text());
        const isLiked = likeButton.hasClass('active'); // 当前是否已点赞

        // 乐观更新：先在 UI 上切换状态
        likeButton.toggleClass('active btn-primary btn-outline-primary');
        likeCountElement.text(isLiked ? currentCount - 1 : currentCount + 1);

        // 发送请求到后端
        $.ajax({
            url: isLiked
                ? '${pageContext.request.contextPath}/scenic/unlike' // 取消点赞
                : '${pageContext.request.contextPath}/scenic/like', // 点赞
            type: 'POST',
    ...
    function toggleFavorite(button) {
        ...
        const scenicId = favoriteButton.data('scenic-id'); // 获取景点ID
        const isFavorited = favoriteButton.hasClass('active'); // 当前是否已收藏

        // **乐观更新：先切换UI状态**
        favoriteButton.toggleClass('active'); // 切换黄色背景

        // 发送请求到后端
        $.ajax({
            url: isFavorited
                ? '${pageContext.request.contextPath}/scenic/unfavorite' // 取消收藏
                : '${pageContext.request.contextPath}/scenic/favorite', // 收藏
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ scenicId: scenicId }),
            success: function(response) {
            ...
            error: function() {
                favoriteButton.toggleClass('active'); // 回滚状态
                alert('请求失败，请检查网络');
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code === 200) {
                    ...
                    loadComments(scenicId,'#commentsBody');
                } else {
